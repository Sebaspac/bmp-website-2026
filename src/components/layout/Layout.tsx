...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div style={{ paddingTop: 60 }} className="min-h-screen flex flex-col bg-background text-foreground font-body">
      ...
      <header style={{
        ...
        {/* LEFT — logo */}
        <div style={{ flex:1, display:'flex', alignItems:'center', padding:'0 24px', height:'100%', borderRight: isMobile ? 'none' : `1px solid ${LINE}` }}>
        ...
        {/* RIGHT — social + lang (desktop) | hamburger (mobile) */}
        {isMobile ? (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', padding:'0 12px', height:'100%' }}>
            ...
            <button
              ...
              style={{ display:'flex', alignItems:'center', justifyContent:'center', width:44, height:44, background:'none', border:'none', cursor:'pointer', padding:0, flexShrink:0 }}
            >
              <div style={{ width:22, height:16, position:'relative' }}>
                <span style={{ position:'absolute', top:0, left:0, width:'100%', height:2, background:MUTED, transformOrigin:'center', transition:'transform .3s ease, opacity .3s ease', transform: mobileMenuOpen ? 'translateY(7px) rotate(45deg)' : 'none' }} />
                <span style={{ position:'absolute', top:'50%', left:0, width:'100%', height:2, background:MUTED, marginTop:'-1px', transition:'opacity .3s', opacity: mobileMenuOpen ? 0 : 1 }} />
                <span style={{ position:'absolute', bottom:0, left:0, width:'100%', height:2, background:MUTED, transformOrigin:'center', transition:'transform .3s ease', transform: mobileMenuOpen ? 'translateY(-7px) rotate(-45deg)' : 'none' }} />
              </div>
            </button>
          </div>
        ) : (
      ...
      {isMobile && (
        <div role="dialog" aria-modal="true" aria-label="Mobile Navigation"
          style={{
            ...
            background:ANTHRA, zIndex:500,
            display:'flex', flexDirection:'column',
            opacity: mobileMenuOpen ? 1 : 0,
            pointerEvents: mobileMenuOpen ? 'all' : 'none',
            transition:`opacity ${mobileMenuOpen ? '.28s' : '.2s'} ease`,
          }}
        >
          {/* Mobile menu top bar */}
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px', height:60, borderBottom:`1px solid ${SEP}`, flexShrink:0 }}>
            <Link to="/" onClick={() => setMobileMenuOpen(false)} style={{ display:'flex', alignItems:'center', textDecoration:'none' }}>
              <img src="/bmp-logo.png" alt="BMP Logo" style={{ height:34, width:'auto', display:'block', objectFit:'contain' }} />
            </Link>
            <button
              onClick={() => setMobileMenuOpen(false)}
              aria-label="Menü schließen"
              style={{ display:'flex', alignItems:'center', justifyContent:'center', width:44, height:44, background:'none', border:'none', cursor:'pointer', color:MUTED, fontSize:28, lineHeight:1, fontFamily:FF_BODY }}
            >×</button>
          </div>
          ...
          <nav style={{ flex:1, overflowY:'auto', padding:'24px 0' }}>
            <ul style={{ listStyle:'none', margin:0, padding:0 }}>
              {NAV.map((item) => (
                <li key={item.key} style={{ borderBottom:`1px solid ${SEP}` }}>
                  ...
                  >
                    {item.label}
                    <span style={{ color:GOLD, fontSize:'1.1em' }}>›</span>
                  </Link>
                </li>
              ))}
            </ul>
            ...
            {/* Mobile CTA buttons */}
            <div style={{ padding:'28px 24px 16px', display:'flex', flexDirection:'column', gap:12 }}>
              <Link to="/teilnahme#bewerben" onClick={() => setMobileMenuOpen(false)}
                style={{ display:'block', background:GOLD, color:'#101828', fontSize: 15, fontWeight:700, textTransform:'uppercase', letterSpacing:'.12em', padding:'14px 24px', textDecoration:'none', fontFamily:FF_DISPLAY, textAlign:'center' }}
              >Jetzt bewerben</Link>
              <Link to="/mitglied-werden" onClick={() => setMobileMenuOpen(false)}
                style={{ display:'block', background:'transparent', color:MUTED, fontSize: 15, fontWeight:700, textTransform:'uppercase', letterSpacing:'.12em', padding:'13px 24px', textDecoration:'none', fontFamily:FF_DISPLAY, border:`1.5px solid rgba(239,191,4,0.35)`, textAlign:'center' }}
              >Mitglied werden</Link>
              <Link to="/formular-hochladen" onClick={() => setMobileMenuOpen(false)}
                style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, background:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.5)', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.14em', padding:'13px 24px', textDecoration:'none', fontFamily:FF_DISPLAY, border:'1px dashed rgba(255,255,255,0.2)', textAlign:'center' }}
            ...
            {/* Mobile social icons */}
            <div style={{ padding:'16px 24px', borderTop:`1px solid ${SEP}`, display:'flex', gap:20 }}>
              {SOCIAL.map(s => <SocialLink key={s.label} {...s} />)}
            </div>
          </nav>
        </div>
