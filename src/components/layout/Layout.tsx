...
/* ── SVG icons ─────────────────────────────────────────── */
const IgIcon = () => <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".8" fill="currentColor" stroke="none"/></svg>;
const LiIcon = () => <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>;
const FbIcon = () => <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>;
const SOCIAL = [
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const [activeKey, setActive] = useState<string | null>(null);  // desktop hover
  ...
  useEffect(() => {
    if (!open) {
      setTimeout(() => { setExpand(null); setActive(null); }, 220);
    }
  }, [open]);
  ...
  useEffect(() => {
    if (open && !isMobile) setTimeout(() => firstLinkRef.current?.focus(), 300);
  }, [open, isMobile]);
  ...
  // Hide floating CTA when form section in view
  useEffect(() => {
  ...
  return (
    <div style={{ paddingLeft: 60 }} className="min-h-screen flex flex-col bg-background text-foreground font-body">
      ...
      <aside style={{
        ...
        {/* Gold vertical line */}
        <div style={{ width:1, background:LINE, margin:'14px 0', flex:1, minHeight:40, maxHeight:'calc(100% - 220px)' }} />
        ...
        >
          <div style={{ display:'flex', flexDirection:'column', gap:4.5, width:20 }}>
            <span style={{ display:'block', height:1.5, background:MUTED, transformOrigin:'center', transition:'transform .3s ease, opacity .3s ease', transform: open ? 'translateY(6px) rotate(45deg)' : 'none' }} />
            <span style={{ display:'block', height:1.5, background:MUTED, transition:'opacity .3s', opacity: open ? 0 : 1 }} />
            <span style={{ display:'block', height:1.5, background:MUTED, transformOrigin:'center', transition:'transform .3s ease', transform: open ? 'translateY(-6px) rotate(-45deg)' : 'none' }} />
          </div>
          <span style={{ fontSize:9, fontWeight:600, textTransform:'uppercase', letterSpacing:'.22em', color:MUTED, lineHeight:1, fontFamily:FF_DISPLAY, transition:'opacity .25s', opacity: open ? 0 : 1 }}>Menü</span>
        </button>

        {/* Social icons */}
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14, marginTop:'auto', flexShrink:0 }}>
          {SOCIAL.map(s => <SocialLink key={s.label} {...s} />)}
        </div>

        {/* Lang switcher */}
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2, marginTop:18, flexShrink:0 }}>
      ...
      >
        ...
        <div style={{
          position: 'absolute', bottom: -20, left: 60, right: 0,
          fontSize: 'clamp(50px,9vw,110px)', fontWeight:900, textTransform:'uppercase',
          letterSpacing:'-.02em', lineHeight:1,
          color: 'rgba(255,255,255,.055)',
          whiteSpace: 'nowrap', overflow:'hidden',
          pointerEvents:'none', userSelect:'none',
          fontFamily: FF_DISPLAY,
        }}>EXZELLENZ HAT EINEN NAMEN.</div>

        {isMobile ? (
          /* ── MOBILE: single-column accordion ──────── */
          <div style={{ display:'flex', flexDirection:'column', height:'100%', paddingLeft:60 }}>

            {/* Top bar */}
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px', height:56, borderBottom:`1px solid ${SEP}`, flexShrink:0 }}>
              <Link to="/" onClick={() => setOpen(false)} style={{ display:'flex', alignItems:'center', textDecoration:'none' }}>
                <img src="/bmp-logo.png" alt="BMP Logo" style={{ height:30, width:'auto', display:'block', objectFit:'contain' }} />
              </Link>
              <button onClick={() => setOpen(false)} aria-label="Menü schließen"
                style={{ display:'flex', alignItems:'center', gap:8, background:'none', border:'none', cursor:'pointer', padding:'8px', minWidth:44, minHeight:44, justifyContent:'center' }}>
              ...
              <ul style={{ listStyle:'none', margin:0, padding:0 }}>
                {NAV.map((item) => (
                  <li key={item.key} style={{ borderTop:`1px solid ${SEP}` }}>
                    {/* Main item: tap to expand subnav */}
                    <button
                    ...
                    {/* Subnav accordion body */}
                    <div style={{
                      overflow:'hidden',
                      maxHeight: expandKey === item.key ? 400 : 0,
                            ...
                            {item.sub.map(sub => (
                              <li key={sub.path} style={{ borderTop:'1px solid rgba(239,191,4,.1)' }}>
                                <Link to={sub.path} onClick={() => setOpen(false)}
                                  style={{ display:'block', padding:'10px 0', textDecoration:'none', color:MUTED, fontSize:14, fontFamily:FF_BODY, minHeight:44, display:'flex', alignItems:'center' } as React.CSSProperties}
                                >{sub.label}</Link>
                              </li>
                            ))}
                          </ul>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', height:'100%', paddingLeft:60 }}>
                  ...
                  <li key={item.key} style={{ borderTop:`1px solid ${SEP}` }}>
                    <Link
                      ...
                      style={{
                        ...
                        transition:'color .15s, opacity .3s ease-out',
                        transform: open ? `translateX(${activeKey === item.key ? 6 : 0}px)` : 'translateX(-16px)',
                        opacity: open ? 1 : 0,
            ...
            <div style={{
              ...
              borderRight:`1px solid ${SEP}`,
              position:'relative', zIndex:1, minHeight:440,
            }}>
              {NAV.map(item => (
                <div key={item.key} style={{
                  position:'absolute', top:80, left:48, right:40,
                  ...
                  <div style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.22em', color:'#009EE0', marginBottom:16, fontFamily:FF_DISPLAY }}>Überblick</div>
                  <div style={{ fontSize:16, fontWeight:700, textTransform:'uppercase', letterSpacing:'.1em', color:'#fff', marginBottom:20, fontFamily:FF_DISPLAY }}>{item.subLabel}</div>
                  <ul style={{ listStyle:'none' }}>
                    {item.sub?.map(sub => (
                      <li key={sub.path} style={{ borderTop:'1px solid rgba(239,191,4,.12)' }}>
                        <Link to={sub.path} onClick={() => setOpen(false)}
                          style={{ display:'block', padding:'9px 0', textDecoration:'none', color:MUTED, fontSize:14, fontFamily:FF_BODY, transition:'color .15s, padding-left .15s' }}
            ...
            {/* RIGHT: utilities */}
            <div style={{ padding:'48px 60px 48px 48px', display:'flex', flexDirection:'column', position:'relative', zIndex:1 }}>

              {/* Top row: close + logo */}
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'auto' }}>
                <button onClick={() => setOpen(false)} aria-label="Menü schließen"
                  style={{ display:'flex', alignItems:'center', gap:10, background:'none', border:'none', cursor:'pointer', padding:0 }}
                  onMouseEnter={e => { (e.currentTarget.querySelector('.ov-x-a') as HTMLElement).style.background = '#fff'; (e.currentTarget.querySelector('.ov-x-b') as HTMLElement).style.background = '#fff'; (e.currentTarget.querySelector('.ov-close-lbl') as HTMLElement).style.color = '#fff'; }}
                  onMouseLeave={e => { (e.currentTarget.querySelector('.ov-x-a') as HTMLElement).style.background = MUTED; (e.currentTarget.querySelector('.ov-x-b') as HTMLElement).style.background = MUTED; (e.currentTarget.querySelector('.ov-close-lbl') as HTMLElement).style.color = MUTED; }}
                >
                  <div style={{ width:28, height:28, position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <span className="ov-x-a" style={{ position:'absolute', width:20, height:1.5, background:MUTED, transform:'rotate(45deg)', transition:'background .2s' }} />
                    <span className="ov-x-b" style={{ position:'absolute', width:20, height:1.5, background:MUTED, transform:'rotate(-45deg)', transition:'background .2s' }} />
                  </div>
                  <span className="ov-close-lbl" style={{ fontSize:9, fontWeight:600, textTransform:'uppercase', letterSpacing:'.22em', color:MUTED, fontFamily:FF_DISPLAY, transition:'color .2s' }}>Menü</span>
                </button>
                <div style={{ textAlign:'right' }}>
                  <img src="/bmp-logo.png" alt="BMP Logo" style={{ height:36, width:'auto', display:'block', opacity:0.9 }} />
                </div>
              </div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:24, marginTop:'auto' }}>
                    ...
                    {(['DE','EN'] as const).map((l, i) => (
                      <React.Fragment key={l}>
                        ...
                        <button onClick={() => setLang(l)}
                          style={{ fontSize:11, fontWeight:600, letterSpacing:'.1em', textTransform:'uppercase', background:'none', border:'none', cursor:'pointer', color:lang===l ? GOLD : MUTED, transition:'color .2s', padding:0, fontFamily:FF_DISPLAY }}
                          onMouseEnter={e => { if (lang !== l) e.currentTarget.style.color = '#fff'; }}
                          onMouseLeave={e => { if (lang !== l) e.currentTarget.style.color = MUTED; }}
                        >{l}</button>
                      </React.Fragment>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {SECONDARY.map(item => (
                    <Link key={item.path} to={item.path} onClick={() => setOpen(false)}
                      style={{ fontSize:13, color:MUTED, textDecoration:'none', fontFamily:FF_BODY, transition:'color .15s', width:'fit-content' }}
                      onMouseEnter={e => (e.currentTarget.style.color = GOLD)}
                      onMouseLeave={e => (e.currentTarget.style.color = MUTED)}
                    >{item.label}</Link>
                  ))}
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  ...
                  >Mitglied werden</Link>
                  <div style={{ height:1, background:'rgba(239,191,4,0.12)', margin:'2px 0' }} />
                  <Link to="/formular-hochladen" onClick={() => setOpen(false)}
      ...
      {/* ── FOOTER ───────────────────────────────────── */}
      <footer style={{ background:'#1A2030', borderTop:`4px solid ${GOLD}`, position:'relative', overflow:'hidden' }}>

        {/* Radial glow */}
        <div style={{ position:'absolute', top:0, left:0, width:'100%', height:'100%', background:'radial-gradient(ellipse 600px 300px at 0% 0%, rgba(239,191,4,0.04) 0%, transparent 60%)', pointerEvents:'none' }} />

        {/* Headline band */}
        <div style={{ position:'relative', padding: isMobile ? '28px 24px 16px' : '36px 64px 0' }}>
          <h2 style={{ fontFamily:FF_DISPLAY, fontWeight:900, fontSize:'clamp(1.6rem, 3vw, 2.8rem)', textTransform:'uppercase', letterSpacing:'-0.03em', color:'#fff', margin:0, lineHeight:1, whiteSpace: isMobile ? 'normal' : 'nowrap' }}>
            Der Preis des <span style={{ color:GOLD }}>Bayerischen</span> Mittelstands
          </h2>
          <div style={{ marginTop:14, display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ width:36, height:1, background:GOLD }} />
            <span style={{ fontFamily:FF_DISPLAY, fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.35em', color:'rgba(239,191,4,0.55)' }}>Exzellenz seit 2005</span>
            <div style={{ flex:1, height:1, background:'rgba(239,191,4,0.18)' }} />
          </div>
        </div>

        {/* Body: asymmetric layout */}
        <div style={{ position:'relative', padding: isMobile ? '24px 24px' : '24px 64px 20px', display:'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 40 : 0, alignItems:'flex-start' }}>

          {!isMobile && (
            <div style={{ position:'absolute', left:52, top:-28, fontFamily:FF_DISPLAY, fontWeight:900, fontSize:160, lineHeight:1, color:'transparent', WebkitTextStroke:'1px rgba(239,191,4,0.1)', userSelect:'none', pointerEvents:'none', zIndex:0 }}>2026</div>
          )}

          {/* Left col: copy + CTAs */}
          <div style={{ position:'relative', zIndex:1, flex: isMobile ? 'none' : '0 0 420px', maxWidth: isMobile ? '100%' : 420 }}>
            <img src="/bmp-logo.png" alt="Bayerischer Mittelstandspreis" style={{ width:140, height:'auto', marginBottom:10, opacity:0.9 }} />
            <p style={{ fontFamily:FF_BODY, fontSize:15, lineHeight:1.65, color:'rgba(255,255,255,0.45)', margin:'0 0 16px 0', maxWidth:300 }}>
              Die renommierteste Auszeichnung für herausragende Unternehmen des bayerischen Mittelstands.
            </p>
            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-start', gap:8 }}>
              <Link to="/teilnahme#bewerben" style={{ fontFamily:FF_DISPLAY, fontSize:16, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.12em', color:'#101828', background:GOLD, padding:'10px 22px', textDecoration:'none', display:'inline-block', borderRadius:2, transition:'background 0.15s, box-shadow 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.background='#FFD130'; e.currentTarget.style.boxShadow='0 0 18px rgba(239,191,4,0.65), 0 0 40px rgba(239,191,4,0.3)'; }}
                onMouseLeave={e => { e.currentTarget.style.background=GOLD; e.currentTarget.style.boxShadow='none'; }}
              >Kostenlos bewerben</Link>
              <Link to="/mitglied-werden" style={{ fontFamily:FF_DISPLAY, fontSize:16, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.12em', color:MUTED, border:'1.5px solid rgba(239,191,4,0.35)', padding:'8px 22px', textDecoration:'none', display:'inline-block', borderRadius:2, transition:'border-color 0.15s, color 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=GOLD; e.currentTarget.style.color=GOLD; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(239,191,4,0.35)'; e.currentTarget.style.color=MUTED; }}
              >Mitglied werden</Link>
              <Link to="/netzwerk#sponsoring" style={{ fontFamily:FF_DISPLAY, fontSize:16, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.12em', color:'rgba(255,255,255,0.4)', textDecoration:'none', paddingTop:4, transition:'color 0.15s' }}
          ...
          {/* Right col: nav links */}
          <div style={{ position:'relative', zIndex:1, flex:1, display:'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: isMobile ? 'flex-start' : 'flex-end', alignItems:'flex-start', gap: isMobile ? 32 : 0, paddingLeft: isMobile ? 0 : 64 }}>
            <div style={{ display:'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 0, alignItems:'flex-start' }}>

              {/* BMP 2026 */}
              <div style={{ minWidth: isMobile ? 'auto' : 160 }}>
                <p style={{ fontFamily:FF_DISPLAY, fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.28em', color:'rgba(255,255,255,0.3)', margin:'0 0 10px 0' }}>BMP 2026</p>
                <ul style={{ listStyle:'none', margin:0, padding:0, display:'flex', flexDirection:'column', gap:8 }}>
                  {[{ l:'Startseite', p:'/' },{ l:'Preisträger', p:'/preistraeger' },{ l:'Teilnahme', p:'/teilnahme' }].map(item => (
                    <li key={item.p}><Link to={item.p} style={{ fontFamily:FF_BODY, fontSize:16, color:'rgba(255,255,255,0.55)', textDecoration:'none', transition:'color 0.2s', display:'inline-block' }} onMouseEnter={e=>{e.currentTarget.style.color='#fff';}} onMouseLeave={e=>{e.currentTarget.style.color='rgba(255,255,255,0.55)';}}>{item.l}</Link></li>
                  ))}
                </ul>
              </div>

              {!isMobile && <div style={{ width:1, alignSelf:'stretch', background:'rgba(239,191,4,0.22)', margin:'0 40px', flexShrink:0 }} />}

              {/* Engagement */}
              <div style={{ minWidth: isMobile ? 'auto' : 160 }}>
                <p style={{ fontFamily:FF_DISPLAY, fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.28em', color:'rgba(255,255,255,0.3)', margin:'0 0 10px 0' }}>Engagement</p>
                <ul style={{ listStyle:'none', margin:0, padding:0, display:'flex', flexDirection:'column', gap:8 }}>
                  {[{ l:'Netzwerk & Jury', p:'/netzwerk' },{ l:'Presse', p:'/presse' },{ l:'Kontakt', p:'/kontakt' }].map(item => (
                    <li key={item.p}><Link to={item.p} style={{ fontFamily:FF_BODY, fontSize:16, color:'rgba(255,255,255,0.55)', textDecoration:'none', transition:'color 0.2s', display:'inline-block' }} onMouseEnter={e=>{e.currentTarget.style.color='#fff';}} onMouseLeave={e=>{e.currentTarget.style.color='rgba(255,255,255,0.55)';}}>{item.l}</Link></li>
                  ))}
                </ul>
              </div>
              ...
              <div style={{ minWidth: isMobile ? 'auto' : 120 }}>
                <p style={{ fontFamily:FF_DISPLAY, fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.28em', color:'rgba(255,255,255,0.3)', margin:'0 0 10px 0' }}>Legal</p>
                <ul style={{ listStyle:'none', margin:0, padding:0, display:'flex', flexDirection:'column', gap:8 }}>
                  {[{ l:'Datenschutz', p:'/datenschutz' },{ l:'Impressum', p:'/impressum' }].map(item => (
                    <li key={item.p}><Link to={item.p} style={{ fontFamily:FF_BODY, fontSize:16, color:'rgba(255,255,255,0.55)', textDecoration:'none', transition:'color 0.2s', display:'inline-block' }} onMouseEnter={e=>{e.currentTarget.style.color='#fff';}} onMouseLeave={e=>{e.currentTarget.style.color='rgba(255,255,255,0.55)';}}>{item.l}</Link></li>
                  ))}
                </ul>
              </div>
        ...
        {/* Partner strip */}
        <div style={{ borderTop:'1px solid rgba(239,191,4,0.15)', borderBottom:'1px solid rgba(239,191,4,0.15)', padding: isMobile ? '20px 24px' : '20px 64px', display:'flex', alignItems:'center', gap: isMobile ? 16 : 28, flexWrap:'wrap' }}>
          <span style={{ fontFamily:FF_DISPLAY, fontSize:8, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.32em', color:'rgba(255,255,255,0.22)', flexShrink:0 }}>Partner</span>
          <div style={{ width:1, height:20, background:'rgba(239,191,4,0.2)', flexShrink:0 }} />
          <img src="/images/ewif-logo.png" alt="EWIF" style={{ height:32, width:'auto', opacity:0.5, filter:'brightness(2)', display:'block' }} />
          <img src="/images/partner-logos.png" alt="Partner" style={{ height:32, width:'auto', opacity:0.5, filter:'brightness(2)', display:'block' }} />
        </div>

        {/* Bottom bar */}
        <div style={{ padding: isMobile ? '20px 24px' : '20px 64px', display:'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', justifyContent:'space-between', gap: isMobile ? 16 : 0 }}>
          <p style={{ fontFamily:FF_DISPLAY, fontSize:10, fontWeight:400, textTransform:'uppercase', letterSpacing:'0.2em', color:'rgba(255,255,255,0.3)', margin:0 }}>
